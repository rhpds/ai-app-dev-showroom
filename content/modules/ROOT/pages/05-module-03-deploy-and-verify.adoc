...
+
You can click on each task to see its logs. The **build-image** task logs will look familiar -- it's building the container image using the same `Containerfile` from Module 2.
+
...
+
// TODO: Screenshot of CI tab in RHDH showing pipeline runs
image::rhdh-ci-tab.png[CI tab in Developer Hub showing pipeline runs,link=self,window=blank,width=700]
...
* The pipeline run is visible in the OpenShift console under **Pipelines** and in Developer Hub under the **CI** tab
...
Once the pipeline completed, confirm that:

* The ArgoCD application is visible and shows **Progressing** status (or **Synced** if you waited too long)
* The health status shows **Healthy**
* You can see the new deployed Kubernetes resources in the ArgoCD dashboard

image::argocd-resource-updates.png[ArgoCD resource tree showing updates,link=self,window=blank,width=700]
...
+
NOTE: If you see a loading error on the Topology tab, do a hard refresh in your browser (kbd:[CTRL+SHIFT+R] or kbd:[CMD+SHIFT+R] on Mac).
+
